...
15. **Single-source project metadata.** The project file's YAML frontmatter (priority, status, due) is the source of truth. The dashboard surfaces active projects via dataview query — no parallel `maps/projects.md` to sync (retired 2026-04-17). When changing how the vault or Claude's systems work (new file types, new workflows, deprecated files, changed conventions), update vault `AGENTS.md`, home `AGENTS.md`, and `MEMORY.md` in the SAME turn.
