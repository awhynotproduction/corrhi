...
1. Read `_claude/approval-diffs.md` — the raw correction log
...
4. **For each cluster, decide tier:**
   - **Behavioral rule** (Claude should DO this differently every time) → Add to ~/CLAUDE.md Behavioral Rules
   - **Memory file** (context Claude should KNOW) → Create/update `_claude/memory/feedback_*.md`
   - **Lens update** (how the user thinks/decides) → Update `_claude/[user]-lens.md`
...
6. **Promote behavioral rules** to ~/CLAUDE.md if:
...
7. **Clear processed diffs** from approval-diffs.md (archive the raw text to _claude/archive/)
...
- When promoting to CLAUDE.md, include the WHY (what failure prompted this rule)
